...
function main(v) {
    let parser = yargs()
        ...
        .help('h')
        .alias('h', 'help');
    // console.info(v.split(','))
    let originArgv = v.split(' ')
    // let originArgv = process.argv.splice(2);
    // console.info(originArgv)
    let argv = parser.parse(originArgv);
    switch (argv.command) {
        case 'completion':
            return parser.showCompletionScript();
            break;
        case 'list':
            return list();
            break;
        default:
            ...
            if (subCommandInfo) {
                return parseSub(subCommandInfo, argv, originArgv.splice(1));
            } else {
                return parser.showHelp();
            }
    }
